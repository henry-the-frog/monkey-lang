    ...
    while (this.currentFrame().ip < this.currentFrame().instructions().length - 1) {
      ...
      switch (op) {
        ...
        case Opcodes.OpIncrementLocal: {
          ...
          this.currentFrame().ip += 1;
          const base = this.currentFrame().basePointer;
          const val = this.stack[base + localIdx];
          if (typeof val === 'number') {
            this.stack[base + localIdx] = val + 1;
          } else if (val && val._cell !== undefined) {
            val._cell = (typeof val._cell === 'number' ? val._cell : val._cell.value) + 1;
          } else {
            this.stack[base + localIdx] = unwrapInt(val) + 1;
          }
          break;
        }
        ...
        case Opcodes.OpAddSetLocal: {
          ...
          if (typeof left === 'number' && typeof right === 'number') {
            this.stack[base + localIdx] = left + right;
          } else {
            this.stack[base + localIdx] = unwrapInt(left) + unwrapInt(right);
          }
          break;
        }
        ...
        case Opcodes.OpAddSetGlobal: {
          ...
          const left = this.pop();
          if (typeof left === 'number' && typeof right === 'number') {
            this.globals[globalIdx] = left + right;
          } else {
            this.globals[globalIdx] = unwrapInt(left) + unwrapInt(right);
          }
          break;
        }
